#!/bin/bash
# Pins "Cervid Ballistics.app" to the Dock.
#
# Usage:
#   chmod +x add_to_dock.sh
#   ./add_to_dock.sh
#
# Requires the app to already be installed (run install.sh / reinstall.sh first).
...
if [ ! -d "$APP_PATH" ]; then
  ...
  if [ -d "/Applications/$APP_NAME.app" ]; then
    ...
    echo "Run install.sh (or reinstall.sh) first, then try this again."
    exit 1
  fi
fi
