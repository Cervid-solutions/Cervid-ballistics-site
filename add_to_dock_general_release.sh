#!/bin/bash
# Pins "Cervid Ballistics (General Release).app" to the Dock.
#
# Usage:
#   chmod +x add_to_dock_general_release.sh
#   ./add_to_dock_general_release.sh
#
# Requires the app to already be installed (run install_general_release.sh first).
...
if [ ! -d "$APP_PATH" ]; then
  ...
  if [ -d "/Applications/$APP_NAME.app" ]; then
    ...
    echo "Run install_general_release.sh first, then try this again."
    exit 1
  fi
fi
